import React from 'react'
import ReactDom from 'react-dom'
import './index.css'

// >  <   =>  = () =>

// function Greeting() {
//   return (
//     <div>
//       <Person></Person>
//       <Message></Message>
//     </div>
//   )
// }
// const Person = () => <h2>Esther DEBA</h2>
// const Message = () => {
//   return <p>Student</p>
// }
// const Greeting = () => {
//   return React.createElement('div', {}, React.createElement('h1', {}, ''))
// }

// FIRST TRY

const firstBook = {
  image:
    'https://m.media-amazon.com/images/I/61TAj716z4L._AC_UY327_FMwebp_QL65_.jpg',
  title: 'Dieu - La science Les preuves - Collector',
  author: 'de Michel-Yves Bolloré, Olivier Bonnassies, et al.',
  date: '| 13 octobre 2022',
  price: '32,00€',
  vari: 'Relié',
  livraison: 'Livraison à 25,99 €',
}
const secondBook = {
  image:
    'https://m.media-amazon.com/images/I/81GxkhREP0L._AC_UY327_FMwebp_QL65_.jpg',
  title: 'Little Women',
  author: 'Édition en Anglais | de Louisa May Alcott | 4 février 2021',
  date: '| 13 octobre 2022',
  vari: 'Broché',
  price: '13,15€ ',
  livraison: 'Livraison à 25,99 €',
}
const thirdBook = {
  image:
    'https://m.media-amazon.com/images/I/41FjeWxj-UL._AC_UY327_FMwebp_QL65_.jpg',
  title: 'The Sadness Book - A Journal To Let Go',
  author: 'Édition en Anglais | de Elias Baar  | 3 mars 2022',
  date: '| 13 octobre 2022',
  vari: 'Broché',
  price: '13,95€',
  livraison: 'Livraison à 25,99 €',
}

const BookList = () => {
  return (
    <div>
      <Book
        image={firstBook.image}
        title={firstBook.title}
        author={firstBook.author}
        date={firstBook.date}
        vari={firstBook.vari}
        price={firstBook.price}
        livraison={firstBook.livraison}
      />
      <Book
        image={secondBook.image}
        title={secondBook.title}
        author={secondBook.author}
        date={secondBook.date}
        vari={secondBook.vari}
        price={secondBook.price}
        livraison={secondBook.livraison}
      />
      <Book
        image={thirdBook.image}
        title={thirdBook.title}
        author={thirdBook.author}
        date={thirdBook.date}
        vari={thirdBook.vari}
        price={thirdBook.price}
        livraison={thirdBook.livraison}
      />
    </div>
  )
}

const Book = (props) => {
  return (
    <div className='center'>
      <article className='container'>
        <img src={props.image} alt='' />
        <div className='text'>
          <div className='header'>
            <p className='sponsor'>Sponsorisé</p>
            <img
              src='data:image/png;base64,'
              alt=''
              className='i'
            />
          </div>

          <h2 className='title'>{props.title}</h2>

          <div className='phrase'>
            <p className='author'>{props.author}</p>
            <p className='date'>{props.date}</p>
          </div>
          <p className='relie'>{props.vari}</p>
          <p className='price'>{props.price}</p>
          <p className='livraison'>{props.livraison}</p>
        </div>
      </article>
    </div>
  )
}

// const Simple = () => {
//   const text = "hello we're here to learn"
//   return (
//     <div>
//       <h1>{text.toLocaleUpperCase()}</h1>
//       <img
//         src='https://m.media-amazon.com/images/I/61TAj716z4L._AC_UY327_FMwebp_QL65_.jpg'
//         alt='couverture du livre'
//       />
//       <h1>{2 + 2}</h1>
//     </div>
//   )
// }

ReactDom.render(<BookList />, document.getElementById('root'))
